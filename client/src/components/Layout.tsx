...
import { Button, IconButton, Modal, Skeleton, ErrorState, Badge } from "./workspace-ui";
...
const groups = [
  ...
  { label: "生活", links: [
    ...
    { to: "/reading", label: "阅读桌面", module: "reading", tone: "violet" },
  ] },
...
export function AppLayout() {
  ...
  const toggleNavigation = () => {
    if (window.matchMedia("(max-width: 900px)").matches) setMobileNavOpen((value) => !value);
  ...
  return (
    <div
      className={classNames("app-shell", appearance === "neo" && "neo-shell", collapsed && "sidebar-collapsed", mobileNavOpen && "mobile-nav-open")}
    ...
    >
      ...
      <button className="mobile-nav-scrim" aria-label="关闭导航" onClick={() => setMobileNavOpen(false)} />
      <aside className="sidebar glass-regular">
        <div className="brand"><div className="brand-mark" aria-hidden="true"><ShipIcon size={25} /></div><div className="brand-copy"><strong>Barry工作台</strong><span>私人云端空间</span></div>{appearance === "neo" ? <span className="brand-edition">NEO / PRIVATE CONTROL DESK</span> : null}</div>
          ...
          {groups.map((group) => (
            <div className="nav-group" key={group.label}>
              ...
              {group.links.map(({ to, label, module, tone }) => (
                <NavLink key={to} to={to} end={to === "/"} data-tone={tone} className={({ isActive }) => classNames("nav-link", isActive && "active")} title={label} onClick={() => setMobileNavOpen(false)}>
...
function NeoModuleEmblem({ module }: { module: ModuleArtworkName }) {
  return <ModuleArtwork module={module} className="neo-reading-emblem" />;
}
