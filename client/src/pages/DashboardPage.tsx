...
import { Badge, Button, EmptyState, ErrorState, PageHeader, Section, Skeleton } from "../components/workspace-ui";
...
const summaryMeta: Record<string, { title: string; route: string; module: ModuleArtworkName; empty: string }> = {
  ...
  reading: { title: "阅读桌面", route: "/reading", module: "reading", empty: "尚未同步微信读书数据" },
};
...
function TimeProgress({ now }: { now: Date }) {
  const progress = getTimeProgress(now);
  const number = new Intl.NumberFormat("zh-CN");
  const clock = new Intl.DateTimeFormat("zh-CN", { hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(now);
  return (
    <section className="time-progress-stack" aria-label="时间进度">
      <article className="time-progress-row time-progress-life">
        <div className="time-progress-heading"><div><span>人生进度</span><p>已走过 {number.format(progress.life.elapsedDays)} / {number.format(progress.life.totalDays)} 天</p></div><strong>{progress.life.percent.toFixed(2)}<small>%</small></strong></div>
        <div className="time-progress-meter" role="progressbar" aria-label="人生进度" aria-valuemin={0} aria-valuemax={progress.life.totalDays} aria-valuenow={progress.life.elapsedDays} aria-valuetext={`已走过 ${number.format(progress.life.elapsedDays)} / ${number.format(progress.life.totalDays)} 天`}><span style={{ transform: `scaleX(${progress.life.percent / 100})` }} /></div>
      </article>
      <article className="time-progress-row time-progress-today">
        <div className="time-progress-heading"><div><span>今日进度（24 小时）</span><p>{clock} · 已过去 {number.format(progress.today.elapsedMinutes)} / {number.format(minutesPerDay)} 分钟</p></div><strong>{progress.today.percent.toFixed(2)}<small>%</small></strong></div>
        <div className="time-progress-meter" role="progressbar" aria-label="今日时间进度" aria-valuemin={0} aria-valuemax={minutesPerDay} aria-valuenow={progress.today.elapsedMinutes} aria-valuetext={`已过去 ${number.format(progress.today.elapsedMinutes)} / ${number.format(minutesPerDay)} 分钟`}><span style={{ transform: `scaleX(${progress.today.percent / 100})` }} /></div>
      </article>
    </section>
  );
}
...
function useMinuteClock(): Date {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      timer = window.setTimeout(() => { setNow(new Date()); schedule(); }, delay);
    };
...
const sourceRoutes: Record<string, string> = { media: "/media", development: "/development", consulting: "/consulting", fitness: "/fitness", diet: "/diet", reading: "/reading" };
