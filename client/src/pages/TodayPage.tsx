...
import { CalendarPlus, Check, Clock, DotsThree, ArrowBendDownRight, Trash, Play, X, ArrowSquareOut } from "../icons";
import { api } from "../api";
import { useWorkspace } from "../WorkspaceContext";
import { addDays, classNames, formatDate, formatDuration, localDate } from "../workspace-utils";
    ...
    <div>
      ...
      <Section title={view === "today" ? formatDate(selectedDate) : view === "week" ? "未来七天" : "历史记录"} description={view === "today" ? "按时间顺序处理；没有时间的事项排在最后。" : "按日期查看计划状态。"}>
        {ordered.length ? <div className="plan-table">{ordered.map((item) => <div className={classNames("plan-table-row", item.status === "done" && "is-done")} key={item.id}>
          <button className="complete-control" aria-label={item.status === "done" ? "已完成" : `完成：${item.title}`} disabled={item.status === "done"} onClick={() => void run(() => api.completePlan(item.id))}>{item.status === "done" ? <Check size={14} /> : item.status === "doing" ? <Play size={12} /> : null}</button>
          ...
          <div className="row-menu-wrap"><button className="icon-button" aria-label={`操作：${item.title}`} onClick={() => setMenu(menu === item.id ? null : item.id)}><DotsThree size={20} /></button>{menu === item.id ? <div className="row-menu"><button onClick={() => { openForm(item); setMenu(null); }}><Clock size={15} />调整时间</button><button onClick={() => void run(() => api.update("planItems", item.id, { status: "doing" }))}><Play size={15} />开始执行</button><button onClick={() => void run(() => api.postponePlan(item.id, addDays(item.plan_date, 1)))}><ArrowBendDownRight size={15} />移到明天</button><button onClick={() => void run(() => api.update("planItems", item.id, { status: "cancelled" }))}><X size={15} />取消</button><button className="danger" onClick={() => void run(() => api.remove("planItems", item.id))}><Trash size={15} />移到回收站</button></div> : null}</div>
...
const sourceLabels: Record<string, string> = { media: "自媒体", development: "开发工作", consulting: "咨询工作", fitness: "健身计划", diet: "饮食计划", reading: "阅读桌面" };
